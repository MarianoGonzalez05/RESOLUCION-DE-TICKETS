...
//UTILIZANDO LA INTERFAZ PRCEDIMENTAL (http://php.net/manual/es/mysqli.quickstart.dual-interface.php)
 ...
 function guardar($objConexion){
   $sql="insert into libros_i(autor,titulo,edicion,año,origen,tipo,area,materia,comentario,archivo)
 ...
 function borrar($objConexion,$nro=0)	// elimina el estudiante
	{
			$sql="delete from libros_i where id_libro=$nro";
...
function traer_datos($objConexion,$nro=0) // declara el constructor, si trae el numero de estudiante lo busca 
	{
		if ($nro!=0)
		{
			$sql="select * from libros_i where id_libro = $nro";
 ...
 static function buscar($objConexion,$str){
    $sql="select * from libros_i where autor like '%$str%' or titulo like '%$str%' or tipo like '%$str%' or Area like '%$str%' or Materia like '%$str%' or id_libro='$str' order by titulo";
    //$rs=mysql_query($sql);
	$rs=mysqli_query($objConexion,$sql);
	$lib=array();
 ...
 static function buscar_tit($objConexion,$str,$tipo){
    ...
    if($tipo=='origen'){
    $sql="SELECT DISTINCT origen FROM libros_i WHERE origen LIKE '".$str."%' ORDER BY origen ASC";
 ...
 function addBook($title, $author, $isbn, $copies) {
...
function loanBook($book_id, $member_id) {
    $conn = new mysqli('localhost', 'username', 'password', 'library_db');
    ...
    $stmt->bind_param("iiss", $book_id, $member_id, $loan_date, $return_date);
    $stmt->execute();
    $stmt->close();
    $conn->close();
}
